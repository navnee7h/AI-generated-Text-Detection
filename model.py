...
def lsvm(data_tfidf):
     model = joblib.load('lsvm_model.pkl')
     predictions = model.predict(data_tfidf)
     return predictions

def rf(data_rf_tfidf):
     model = joblib.load('rf_model.pkl')
     predictions = model.predict(data_rf_tfidf)
     return predictions
...
class GPT2PPL:
    ...
    def getResults(self, threshold):
        if threshold < 60:
            label = 0
            return "AI Written", label
        elif threshold < 80:
            label = 0
            return "The Text most probably contain parts which are generated by AI and are intervened by Human", label
        else:
            label = 1
            return "Human Written", label
    ...
    def __call__(self, sentence):
        ...
        if total_valid_char == 0:
            results = OrderedDict({
            "Perplexity": 0,
            "Perplexity per line": 0,
            "Burstiness": 0,
            "Result": "Please input some text."
            })
            out = 0
            results["status"] = "Please input some text."
            return results, "Please input some text."
        ...
        if total_valid_char < 100:
            results["status"] = "Please input at least 100 characters."
            results["Perplexity"] = ppl
            results["Perplexity per line"] = Perplexity_per_line
            results["Burstiness"] = max(Perplexity_per_line)
            results["Result"] = "Please input min 100 characters."
            return results, "Please input min 100 characters."
